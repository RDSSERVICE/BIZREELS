...
class CreatorMarketplaceController {
  ...
  discover = asyncHandler(async (req, res) => {
    ...
    const creators = rawCreators.map((c) => {
      ...
      return {
        ...
        city: c.city || profile.location?.city || 'Mumbai',
        state: profile.location?.state || c.location?.state || 'Maharashtra',
        country: profile.location?.country || c.location?.country || 'India',
  ...
  getCreatorProfile = asyncHandler(async (req, res) => {
    ...
    const formattedProfile = {
      ...
      city: creator.city || cp.location?.city || 'Mumbai',
      state: cp.location?.state || creator.location?.state || 'Maharashtra',
      country: cp.location?.country || creator.location?.country || 'India',
      ...
      rating_avg: creator.rating_avg ?? 0,
