...
class WalletService {
  ...
  // ─── Request Payout (Merchant Fiat Sales Revenue) ─────────
  async requestPayout({ userId, amount }) {
    ...
    if (requestedPaise > availablePaise) {
      throw ApiError.badRequest(`Withdrawal amount cannot exceed available sales earnings. Available: ₹${(availablePaise / 100).toFixed(2)}, Requested: ₹${numAmt.toFixed(2)}`);
    }
    ...
    await cache.deleteCache(`wallet:balance:${uid}`);
    this._emitWalletUpdate(uid, wallet.credits, 'debit', numAmt, `Payout withdrawal request for ₹${numAmt}`);
