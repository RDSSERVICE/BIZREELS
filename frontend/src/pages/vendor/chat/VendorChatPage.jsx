...
import {
  FiMessageSquare, FiUser, FiVideo, FiSend, FiCheck,
  FiSearch, FiPaperclip, FiPhoneCall, FiMoreVertical, FiPlusSquare
} from 'react-icons/fi';
...
const TABS = [
  { key: 'customers', label: 'Customer Messages', icon: FiUser },
  { key: 'creators', label: 'Creator Chats', icon: FiVideo },
];
...
export default function VendorChatPage() {
  ...
  const liveThreads = conversationsList.map((c) => {
    ...
    const name = other.name || other.shopName || other.businessName || 'Customer';
  ...
  const { data: msgData, isFetching: isMsgLoading, refetch: refetchMessages } = useGetMessagesQuery(
    ...
    { skip: !selectedThreadId, pollingInterval: 300000 }
  );
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col gap-6 animate-fade-in">
      ...
      <AdminPageHeader
        ...
        subtitle="Interact in real-time with customers and creators inquiring about your listings"
      />
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {isConvLoading && filteredThreads.length === 0 ? (
              ...
              <div className="p-6 text-center text-xs text-text-tertiary space-y-2">
                ...
                <p className="text-[11px]">Customers or creators who message you will appear here.</p>
              </div>
            ) : (
        ...
        {filteredThreads.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-surface/80 gap-4">
            ...
            <div className="max-w-md space-y-1">
              <h3 className="text-base font-bold text-text-primary font-display">No Inquiries Yet</h3>
              <p className="text-xs text-text-tertiary">
                When a customer or creator messages you, you'll be able to chat with them here in real-time.
              </p>
            </div>
          </div>
        ) : (
