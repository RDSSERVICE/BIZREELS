...
export default function AiReelGeneratorModal({ isOpen, onClose, refetch, createReel }) {
  ...
  return (
    ...
    >
      {/* STEP 1: INPUT PROMPT */}
      {step === 'input' && (
        <form onSubmit={handleGenerateAiReel} className="space-y-4 text-left font-sans">
          ...
          <textarea
            ...
            className="w-full p-4 bg-[#1c1d22] border border-white/15 rounded-2xl text-xs text-slate-100 placeholder-slate-400 focus:outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 transition leading-relaxed"
          />
          ...
          <button
            type="submit"
            className="w-full py-3.5 rounded-full bg-gradient-to-r from-amber-500 via-amber-500 to-amber-600 text-white font-extrabold text-xs uppercase tracking-wider shadow-lg shadow-amber-500/25 flex items-center justify-center gap-2 hover:scale-[1.01] active:scale-[0.99] transition-all duration-200 cursor-pointer border border-amber-400"
          >
            <FiCpu size={15} />
      ...
      {step === 'generating' && (
        <div className="py-12 flex flex-col items-center justify-center text-center space-y-6 font-sans">
          <div className="relative flex items-center justify-center">
            <div className="w-16 h-16 rounded-full border-4 border-amber-500/20 border-t-amber-500 animate-spin" />
            <FiZap size={24} className="text-amber-400 absolute animate-pulse" />
          </div>
          <div className="space-y-1">
            <h4 className="text-sm font-bold text-white uppercase tracking-wider">Ad Magic in Progress</h4>
            <p className="text-xs text-slate-300 h-8 animate-fade-in font-medium max-w-sm mx-auto">
      ...
      {step === 'preview' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left font-sans">
          ...
          <div className="space-y-3">
            ...
            <div className="aspect-[9/16] bg-black border border-white/15 rounded-2xl overflow-hidden relative shadow-md">
              <img
              ...
              />
              <div className="absolute top-3 left-3 bg-amber-500 text-white text-[9px] font-extrabold px-2.5 py-1 rounded-full uppercase tracking-wider flex items-center gap-1 shadow-md">
                <FiZap size={10} />
          ...
          <div className="flex flex-col justify-between space-y-4">
            <div className="space-y-4">
              <div>
                <label className="text-[10px] font-extrabold text-amber-300 uppercase tracking-wider block mb-1.5">
                ...
                <textarea
                  ...
                  className="w-full p-4 bg-[#1c1d22] border border-white/15 rounded-2xl text-xs text-slate-100 focus:outline-none focus:border-amber-500 transition leading-relaxed"
            ...
            <div className="flex flex-col gap-2.5 pt-4 border-t border-white/10">
              <button
                ...
                className="w-full py-3.5 rounded-full bg-gradient-to-r from-amber-500 via-amber-500 to-amber-600 text-white font-extrabold text-xs uppercase tracking-wider shadow-lg shadow-amber-500/25 flex items-center justify-center gap-1.5 hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 transition-all duration-200 cursor-pointer border border-amber-400"
              >
                <FiSend size={14} />
              ...
              <div className="flex gap-2.5">
                <button
                  ...
                  className="flex-1 py-3.5 rounded-full bg-white/10 border border-white/10 text-slate-200 hover:bg-white/15 hover:text-white text-xs font-bold transition flex items-center justify-center gap-1 cursor-pointer"
                >
                  <FiArrowLeft size={13} />
