...
import {
  ...
  FiChevronRight, FiStar, FiRefreshCw, FiMic, FiMicOff
} from 'react-icons/fi';
...
export default function VendorListingsPage() {
  ...
  // Fetch vendor current listing coordinates on mount
  useEffect(() => {
  ...
  // ── AI SERVICE DESCRIPTION GENERATOR STATE ──
  ...
  const handleGenerateAiDescription = async (e) => {
    ...
    if (!aiPrompt.trim()) {
      return toast.error('Please enter or dictate a prompt for AI generation');
    }
    ...
    try {
      const res = await api.post('/v1/ai/generate-listing-content', {
        ...
        type: 'service',
        category_name: serviceForm.category,
        sub_category_name: serviceForm.subcategory,
        hints: promptText,
      });

      const responseObj = res.data?.data || res.data || {};
      if (responseObj.ok === false) {
        throw new Error(responseObj.error || 'AI generation failed');
      }

      const gen = responseObj.generated || {};
      const generatedShort = gen.short_description || promptText.slice(0, 50);
      ...
      console.error('Real-time AI backend error:', err);
  ...
  const toggleVoiceRecording = () => {
    ...
    try {
      ...
      recognition.onstart = () => {
        ...
        toast.success('🎙️ Listening... Speak service details now');
      };
          ...
          {activeTab === 'products' ? (
            ...
            <form onSubmit={handleCreateService} className="space-y-6">
              ...
              <div className="space-y-3 p-4 bg-surface-secondary rounded-2xl border border-border">
                ...
                {/* AI DESCRIPTION GENERATOR ASSISTANT BOX */}
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label className="text-[10px] font-bold text-text-tertiary block">Detailed Description (Manual edit or AI Generated)</label>
                    <button
                      type="button"
                      onClick={handleGenerateAiDescription}
                  ...
                  <textarea
                    ...
                    placeholder="Comprehensive service breakdown... (Type manually or use AI generator above)"
                    className="w-full p-2 bg-surface border rounded-xl text-xs"
                  />
                </div>
              </div>
