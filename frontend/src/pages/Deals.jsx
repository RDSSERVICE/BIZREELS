...
export default function Deals() {
  ...
  useEffect(() => { dealApi.mine().then(({ data }) => setItems(data.items || [])).finally(() => setLoading(false)); }, []);
...
function List({ items, loading }) {
  ...
  return (
    <div className="space-y-3" data-testid="deals-list">
      {items.map((d) => (
        <Link key={d.id} to={`/chat/${d.thread_id}`} data-testid={`deal-${d.id}`} className="glass rounded-2xl p-4 flex items-center justify-between hover:bg-white/10">
          <div>
