  ...
  const handleGenerateSpecs = async () => {
    ...
    try {
      ...
      if (res.data && res.data.ok) {
        ...
        const safeTruncateText = (str, maxLen) => {
        ...
        const truncated = safeTruncateText(rawSpecs, 2800);
        setDetailedSpecifications(truncated);
        if (!description.trim()) {
          setDescription(safeTruncateText(rawSpecs, 1400));
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createRequirement({
        type,
        requirementType: type,
        title: safeTruncateText(title, 120),
        category: category === 'Other' ? customCategory : category,
        subcategory: subcategory === 'Other' ? customSubcategory : subcategory,
        ...
        description: safeTruncateText(description, 1400),
        otherConditions: otherConditions ? safeTruncateText(otherConditions, 500) : null,
        photos,
        video,
        detailedSpecifications: detailedSpecifications ? safeTruncateText(detailedSpecifications, 2800) : null,
