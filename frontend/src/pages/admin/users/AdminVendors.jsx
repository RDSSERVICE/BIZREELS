    ...
    <div className="max-w-7xl mx-auto flex flex-col gap-6 animate-fade-in pb-12">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-[#e3dccb] bg-[#f8f4ec]">
                <th className="px-4 py-3.5 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest">Vendor & Shop</th>
                <th className="px-4 py-3.5 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest">Vendor ID</th>
                ...
                <th className="px-4 py-3.5 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest">Products / Active</th>
                <th className="px-4 py-3.5 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest">Joined / Last Login</th>
                <th className="px-4 py-3.5 text-right text-[10px] font-black text-slate-500 uppercase tracking-widest">Settled Sales</th>
            ...
            <tbody className="divide-y divide-[#e3dccb]/50">
              {loadingVendors ? (
                ...
                vendors.map((v) => {
                  ...
                  return (
                    <tr key={v.id} className="transition-colors hover:bg-[#fbf9f4]">
                      {/* Name / Shop Contact */}
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2.5">
                          {v.profile_pic ? (
                            <img src={v.profile_pic} alt={v.name} className="w-8 h-8 rounded-full object-cover border border-[#e3dccb]" />
                          ) : (
                          ...
                          <div>
                            <span className="font-bold text-[#1a1a1a] block text-xs leading-tight">{v.vendorProfile?.store_name || v.name || 'Unknown'}</span>
                            <span className="text-[10px] text-slate-400 mt-0.5 block">{v.phone || v.email || '—'}</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <span className="text-xs font-black text-emerald-600">₹{(v.total_sales || 0).toLocaleString('en-IN')}</span>
                      </td>
        ...
        <form onSubmit={handleResetPassword} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 bg-white border border-[#e3dccb] rounded-xl text-xs font-semibold text-[#1a1a1a] placeholder:text-slate-400 focus:outline-none focus:border-[#1D4ED8] focus:ring-2 focus:ring-[#1D4ED8]/20 transition-all shadow-2xs"
              required
            />
          </div>
          <button
            ...
            className="w-full py-2.5 rounded-xl bg-[#0F172A] hover:bg-[#1E293B] text-white text-xs font-black shadow-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer"
          >
      ...
      >
        <form onSubmit={handleRejectKycSubmit} className="space-y-4">
          <div>
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-1">Reason for Rejection</label>
            <textarea
              placeholder="Specify mismatch or missing fields..."
              value={rejectReason}
              onChange={(e) => setRejectReason(e.target.value)}
              className="w-full px-3.5 py-2.5 bg-white border border-[#e3dccb] rounded-xl text-xs font-semibold text-[#1a1a1a] placeholder:text-slate-400 focus:outline-none focus:border-[#1D4ED8] focus:ring-2 focus:ring-[#1D4ED8]/20 min-h-[100px] transition-all shadow-2xs"
              required
            />
          </div>
          <button
            type="submit"
            disabled={rejecting}
            className="w-full py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-black shadow-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer"
          >
