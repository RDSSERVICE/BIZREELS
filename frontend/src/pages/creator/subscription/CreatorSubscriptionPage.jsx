...
import { useChangeSubscriptionMutation } from '../../../features/vendor/vendorApi';
import { getSocket } from '../../../lib/socket';

export default function CreatorSubscriptionPage() {
  const { data: subData, isFetching: loadingSub, refetch: refetchSub } = useGetCreatorSubscriptionQuery(undefined, { pollingInterval: 10000 });
  ...
  const activePlanName = subData?.data?.plan || subData?.data?.subscription?.planName || subData?.data?.subscription?.plan || subData?.plan || subData?.subscription?.planName || 'Free Member';
  const plans = plansData?.data?.items || plansData?.items || [];

  // Filter plans targeted at creators (or 'all')
  const creatorPlans = plans.filter(p => p.is_active && !p.is_archived && (p.user_type === 'creator' || p.target_role === 'creator' || p.user_type === 'all' || p.target_role === 'all'));

  const handleSubscribe = async (plan) => {
    const confirm = window.confirm(`Confirm membership subscription to ${plan.title} for ₹${plan.price_inr}/${plan.billing_cycle}?`);
    if (!confirm) return;
  ...
  const currentPlanDetails = creatorPlans.find(p => p.title.toLowerCase() === activePlanName.toLowerCase()) || {
    features_list: ['Shoot Earnings Withdrawal', 'Basic Profile Showcase'],
  };
    ...
    <div className="max-w-7xl mx-auto flex flex-col gap-6 animate-fade-in text-xs">
      ...
      <div className="glass rounded-2xl p-6 border border-white/50 shadow-card flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <h3 className="text-xl font-black text-text-primary font-display mt-1 capitalize">{activePlanName}</h3>
          <p className="text-xs text-text-tertiary mt-1">
            {currentPlanDetails.features_list?.join(' • ') || 'Access to basic creator tools'}
          </p>
        </div>
      </div>
      ...
      <div>
        ...
        {loadingPlans ? (
          <div className="text-center py-12 text-text-tertiary animate-pulse">Loading membership plans...</div>
        ) : creatorPlans.length === 0 ? (
          <div className="text-center py-12 text-text-tertiary">No creator plans configured by the admin yet.</div>
        ) : (
                ...
                >
                  ...
                  <ul className="text-xs text-text-secondary space-y-2 pt-1 border-t border-border">
                    <li className="flex items-center gap-2">
                      ...
                      <span>AI Scripting Credits: <strong>{plan.ai_credits || 0} credits</strong></span>
                    </li>
                    <li className="flex items-center gap-2">
                      <FiCheck className="text-emerald-500 flex-shrink-0" />
                      <span>Reels upload limit: <strong>{plan.reels_limit ?? 'Unlimited'}</strong></span>
                    </li>
                  ...
                  >
                    {isCurrent ? 'Active Membership' : 'Upgrade Membership'}
                  </button>
                </div>
              );
            })}
