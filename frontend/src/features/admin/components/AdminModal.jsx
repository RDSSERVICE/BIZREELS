...
export default function AdminModal({ isOpen, onClose, title, children, maxWidth = 'max-w-lg' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 font-sans">
      ...
      {/* Modal Window */}
      <div className={`relative w-full ${maxWidth} max-h-[95vh] sm:max-h-[85vh] bg-[#24262d] text-slate-100 border border-amber-500/30 rounded-t-3xl sm:rounded-3xl shadow-2xl shadow-slate-950/60 animate-scale-in flex flex-col overflow-hidden`}>
        {/* Header */}
        <div className="flex items-center justify-between px-5 sm:px-7 py-4 border-b border-amber-500/25 bg-[#2b2d36] flex-shrink-0">
          <h3 style={{ fontFamily: "'Outfit', sans-serif" }} className="text-sm sm:text-base font-extrabold text-amber-300 tracking-wide truncate pr-2">
            {title}
          </h3>
          <button
            onClick={onClose}
        ...
        {/* Body */}
        <div className="px-5 sm:px-7 py-5 overflow-y-auto flex-1 overscroll-contain text-slate-200 bg-[#24262d]">
          {children}
        </div>
      </div>
    </div>
